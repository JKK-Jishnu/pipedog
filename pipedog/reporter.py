...
<p class="report-path">
  ...
  · Generated by Pipedog v{__version__}
</p>
...
def open_last_report(profile: Optional[str]) -> None:
    """
    Open the most recently modified Excel report with the system default app.
    ...
    if not reports:
        raise FileNotFoundError(
            f"No Excel reports found in {reports_dir}. "
            "Run `pipedog scan` first."
        )
    path = str(reports[-1].resolve())
